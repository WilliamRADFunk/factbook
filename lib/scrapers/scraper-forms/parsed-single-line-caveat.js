...
function parsedSingleLineCaveat(origParams, dataId, hasProp, instProp, baseOntProp, storeKey, dataPropName, label, delimiter, upto) {
    ...
    origParams.cheerioElem(dataId).each(function (index, element) {
        ...
        if (rawScrapedList) {
            var splitList = rawScrapedList.substring(0, rawScrapedList.indexOf(upto)).split(delimiter).map(function (x) { return x.replace(/\\n/g, '').trim(); });
...
exports.parsedSingleLineCaveat = parsedSingleLineCaveat;
;
